				...
				for (var i in session.stories) {
					if (stories[i].tense == 1 && stories[i].finalScore != 0) {
						if (stories[i].doesExist) {
							JiraManager.updateJiraIssue(session.jiraUrl, session.jiraLogin, session.jiraPassword, stories[i].id, stories[i].finalScore);
						} else {
							JiraManager.createJiraIssue(session.jiraUrl, session.jiraLogin, session.jiraPassword, session.jiraProjectKey, stories[i].summary, stories[i].description, stories[i].finalScore);
						}
					}
				}
			}
